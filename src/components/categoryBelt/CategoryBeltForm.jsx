...
import { apiUpdateCollectionById } from "@/services/collectionService";
...
const CategoryBeltForm = ({ initialData }) => {
  ...
  const { register, handleSubmit, watch, setValue, getValues, onChange } =
    ...
    const getCollection = async() => {
      ...
      if(data?.success) setCollection(data?.lotteries)
    }
  ...
  const onSubmit = async (values) => {
    try {
      setIsLoadding(true);
      if(location.pathname === "/categoryBelt/new") {
        if(!values.name || !values?.roomInputs) {
          toast.error("Vui lòng điền tên danh mục và chọn phòng")
          return
        }
        const res = await apiCreateCategoryBelt({
          name : values?.name,
          roomInputs : values?.roomInputs
        });
        console.log(res)
        if (res?.message === "Successfully created") {
          setIsLoadding(false);
          toast.success(`Tạo danh mục thành công `);
          navigate("/categoryBelt");
        }
      } else{
        const res = await apiUpdateCategoryBeltById(id, {
          name : values?.name,
          roomInputs : values?.roomInputs
        });
        console.log(res)
    ...
    <div className="p-10">
      {initialData ? (
        <div className="flex items-center justify-center gap-4">
          <p className="text-2xl font-semibold items-center">Chỉnh sửa danh mục đánh giá</p>
          <Delete item="categoryBelt" id={initialData?._id} />
        </div>
      ) : (
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        ...
        <div className="flex gap-4 flex-col">
          <div>
            ...
            <div className="flex items-center gap-2 text-blue-500 font-semibold">Phòng đã tồn tại : {categoryBelt?.rooms?.map((room) => (
              <p key={room?._id}>
                {room?.room},
              </p>
            ))}</div>
          ...
          >
            {collection && collection?.map((col) => (
              <option value={col?._id} className="py-1" key={col?._id}  selected={selectedCategories.includes(col._id)}>{col?.room}</option>
            ))}
          </select>
...
export default CategoryBeltForm;
